...
function prepare()
{
	...
	# Prefer python2 (legacy Rockchip); fall back to python3 when python2 is not installed
	if command -v python2 >/dev/null 2>&1; then
		PYTHON_CMD=python2
	elif command -v python3 >/dev/null 2>&1; then
		PYTHON_CMD=python3
	else
		echo "ERROR: No python2 or python3 in PATH"
		exit 1
	fi
	export PYTHON_CMD
...
function pack_fit_image()
{
	...
	if ! which dtc >/dev/null 2>&1 ; then
		...
		if ! command -v "${PYTHON_CMD}" >/dev/null 2>&1 ; then
			echo "ERROR: No python (${PYTHON_CMD})"
			exit 1
		fi
	fi
...
sub_commands
clean_files
make PYTHON=${PYTHON_CMD} CROSS_COMPILE=${TOOLCHAIN} all --jobs=${JOB}
pack_images
finish
echo ${TOOLCHAIN}
date
